...
## 工作风格
- 沟通风格要求：必须客观、冷静、实事求是。严禁使用拍马屁、浮夸、过度兴奋的修辞词汇（例如“非常棒”、“顶级”、“完美”等）。专注于技术事实陈述。
- 先用只读工具收集情报, 再制定行动方案
- 主动使用工具获取所需信息, 不要假设或编造数据
- 执行命令前, 在工具调用的 reason 中说明原因
- 对长篇输出使用标准 Markdown 表格和层级标题 (h1, h2, h3)排版，确保终端渲染清晰整洁。

## 入库追问机制 (Copilot UI)
- ⚠️ 当你通过非标准化探测（如 tcpdump/tshark 嗅探包分析、被动监听等）抓取或发现了任何存活的设备资产（如带有明确属性特征的 IP、MAC、设备名称 MacBook/Xiaomi 等），**严禁自行决定入库**。
- 对于所有这类非标准扫描发现的游离资产，你必须在汇报末尾，平稳、客观地向用户逐一提问："在分析中发现了高价值游离资产（[列出核心IP]），是否需要为其授权，由我自动录入当前资产数据库？"
- 在获得用户明确肯定的指示后，主动调用 `claw_record_asset` 工具将您发现的这些资产全部落库。
"""
...
TOOLS = [
    ...
    {
        "type": "function",
        "name": "claw_record_asset",
        "description": "将新发现的资产信息独立写入系统数据库（适用于未经过正式 nmap 扫描流程的零散探测）。需包含 IP，尽量补齐 os (含名称) 和 mac。",
        "parameters": {
            "type": "object",
            "properties": {
                "ip": {
                ...
                "os": {
                    "type": "string",
                    "description": "系统指纹或设备特征名 (如 'macOS', 'Xiaomi Box')"
                },
                "mac": {
                    "type": "string",
                    "description": "MAC 地址 (可选)"
                }
            },
            "required": ["ip"]
        }
...
def tool_record_asset(args: dict) -> str:
    """写入新发现的零散资产"""
    ip = args.get("ip")
    if not ip:
        return json.dumps({"error": "缺少必须参数: ip"})
    os_info = args.get("os", "Unknown")
    mac = args.get("mac", "")
    ...
    try:
        ...
        import uuid
        scan_id = f"ai_intel_{uuid.uuid4().hex[:8]}"

        conn.execute("INSERT INTO scans (scan_id, mode, env) VALUES (?, 'ai_intel', ?)", (scan_id, env))
        conn.execute(
            "INSERT OR REPLACE INTO assets (ip, os, mac, scan_id) VALUES (?, ?, ?, ?)",
            (ip, os_info, mac, scan_id)
        )
        conn.commit()
        conn.close()
        return json.dumps({"status": "success", "message": f"资产 {ip} 已记录到环境 {env}"}, ensure_ascii=False)
    except Exception as e:
        return json.dumps({"error": f"写入资产失败: {str(e)}"})
...
TOOL_DISPATCH = {
    ...
    "claw_record_asset": tool_record_asset,
