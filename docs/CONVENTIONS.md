...
### 4.6 Agent 交互话术与 Markdown 终端格式公约 (Copilot UI)
- **前端排版引擎**: 所有 Agent 输出必须兼容标准 Github Flavored Markdown (GFM)。UI 基于 `react-markdown` 进行定制化抽象挂载。
- **色彩与字体底线**:
  - `h1`: `#FF9900`（琥珀橘），大小 `16px`，带底部描线。
  - `h2`: `#00FFFF`（深青蓝），大小 `14px`。
  - `h3`: `#D0D0D0`（微白灰），大小 `13px`。
  - `table`: 采用深色无明显边框设计（`.data-table`），横向外围包裹可滑动视窗。
  - `code`（行内）: 黑底 `#1A1A1A` 搭配琥珀色 `#FF9900`。
- **对话人设原则**: 客观、冷静、简练。严格禁止使用阿谀奉承、过度夸张（如“顶级”、“完美”、“极其高危”）的情绪化修辞。就事论事汇报战况。
- **自动同步询问机制 (Option 3 规范)**: Agent 在被动嗅探/局域网探测阶段发现任何具备情报价值、但未录入战区大盘的闲散资产（如识别出带有 Mac/主机名的机器）时，**禁止**无提示自动入库。必须在报告末尾使用客观话术主动询问用户：“发现部分目标未在资产库中，是否需要将其正式录入数据库？”在用户明确授权后，方可通过专用登记工具录入。
