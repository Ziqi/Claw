...
- [x] **架构断根**：从 `backend/main.py` 彻底清理 `/api/v1/ops/run` 长寿命挂起调度及其它所有攻击型端点。
- [x] **去武器化**：从 Web 端移除 `A2UI 钓鱼锻造场` 和 `OSINT 实体发包终端`。
- [x] **前端洗牌**：物理删除 `TacticalArmoryModal` 与相关的 UI 入口，结束“一键发炮”时代。
- [x] **文档对齐**：全量架构文档与指南被重构为纯态势感知系统说明。

---

### ⭐ V9.2 — 深度自动化 (Deep Autonomy) (03-29)
> **纵向挖掘 Gemini 3 API 极限，横向打通全域探测雷达的统一武器槽管线**

**【🔴 P0 战役管线填补 (皆已完成)】**
- [x] **ALFA 物理层雷达入列**：建立基于 BSSID 的目标认知网络，在 `mcp_armory_server.py` 接挂底层的 `wifi_nodes` 微架构。
- [x] **全局多选准星 (Global Target Reticle)**：修复武库陷入“单机盲狙”的 UX 欠债，实现跨越 WebUI 多维阵列。
- [x] **Markdown 流式状态机防撕裂**：彻底铲除正则匹配弊病，消除前端遇到未闭合代码段导致的渲染崩溃。
- [x] **React 渲染引擎解耦**：剥离级联状态，消灭 OOM 和 504 熔断。
...
### ⭐ V9.1 — Hacker Copilot 护城河加固 (03-28)
...
### 🚧 Phase 4: C2 集成与情报 (部分废弃)
- ~~Sliver C2 gRPC API 接入 + Session 管理~~ ❌ V9.3 Final Purge 废弃
- ~~Ligolo-ng 隧道管理界面~~ ❌ V9.3 Final Purge 废弃
- Deep Research Agent (异步 OSINT Delegation) — 待评估
- ~~Eavesdropping Shell (xterm.js 人机共驾)~~ ❌ V9.3 XTermConsole 已删除
- 渗透报告一键生成 (PTES/OWASP) — 待开发

### 待用户操作的环境依赖项

| 编号 | 任务 | 前置条件 |
|---|---|---|
| T1 | `make test` 靶场验证 | 需 Kali VM 运行中 |
...
### ~~P6: Sliver C2~~ ❌ 已于 V9.3 Final Purge 废弃
- ~~开源, Go, mTLS/WireGuard~~
- ~~EDR 绕过研究~~
- ~~分布式扫描节点~~

> V9.3 确立了“做减法”原则，CLAW 不再集成 C2 框架，所有后渗透工具在 Kali 终端手动执行。
...
| `12-nuclei-integration` | v5.0.1 | ✅ | ✅ Kali VM 原生 |
| `db_engine` | v5.0 | ✅ | ✅ |
...
| `make toolbox` | v5.0.1 | ✅ | ✅ Kali VM 原生 |
| `scope_check` | v4.0 | ✅ | ✅ |
| `make test` | v4.0 | ✅ | ⚠️ 需靶场网络 |
